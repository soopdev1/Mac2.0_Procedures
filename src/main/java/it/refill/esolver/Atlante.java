...
import static it.refill.esolver.ESolver.pathin;
import static it.refill.esolver.ESolver.pathout;
...
import static it.refill.esolver.ESolver.tag_TES;
...
import static it.refill.esolver.Util.pattern4;
...
import static it.refill.esolver.Util.verificaClientNumber;
import java.io.File;
...
//                String s14 = df.getFatelet();
    ...
    public static void rilasciaAtlante() {
...
//        boolean release = false;
        ...
        if (!release) {
            ...
            if (!atl_ok.isEmpty()) {
                try {
                    File zippato = new File(pathout + "PROD_" + dt.toString(pattern4) + ".7z");
                    SevenZip.compress(atl_ok, pathout, "PROD_" + dt.toString(pattern4) + ".7z");
                    if (zippato.length() > 0) {
                        if (mail) {
